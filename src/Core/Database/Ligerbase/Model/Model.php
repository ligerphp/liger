...
use Core\Database\Ligerbase\Concerns\GuardsAttributes;
use Core\Database\Ligerbase\Model\Builder;
use Core\Database\Ligerbase\Model\Exceptions\FillableNotSetException;
use Core\Database\Ligerbase\Model\Exceptions\UsingAutoIncrementException;
...
class Model extends Builder{
  ...
  protected $_modelName, $_validates=true,$_validationErrors=[],$primaryKey = 'id';
  public $ignore = [],$autoincrement = true,$_results,$timestamps;
  protected static $_db, $_table, $_softDelete=false,$attributes,$connection,$dateFormat = 'Y-M-D-h_m_s';
  const  DELETED_AT = '',RETRIVED_AT = '';
  ...
  public function getColumnsForSave(){
    ...
    foreach($columns as $column){
      ...
      if($key == 'deleted_at' || $key == 'updated_at' || $key == 'created_at'){
        ...
        //check if upadating or creating new row
      ...
      if($this->isFillable($key)){
        ...
        $fields[$key] = $this->{$key};
          ...
          throw new FillableNotSetException($key.' should be added to the fillable property before saving in '.static::$_table .' table');
      }
  ...
  /**
   * Check if user is using auto increment
   * 
   */
  public function isAutoIncrement(){

    if(!$this->autoincrement && empty($this->id)){
      throw new UsingAutoIncrementException('set the value of your primary key, or set autoincrement to true');
    }
  }
