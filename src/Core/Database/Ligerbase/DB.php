<?php
namespace Core\Database\Ligerbase;
use \PDO;
use \PDOException;
...
class DB {
  ...
  public function update($table, $id, $fields = []) {
    ...
    $sql = "UPDATE {$table} SET {$fieldString},updated_at = {$now} WHERE id = {$id}";
    if(!$this->query($sql, $values)->error()) {
      return true;
    }
    return false;
  ...
  public function get_columnsAndExtract($table) {
    $rows  = $this->query("SHOW COLUMNS FROM {$table}")->results();
